...
**Goal**: Display system banner messages BELOW the header, while keeping permanent banners (extension update) ABOVE the header.
...
(defn app-header
  "Common header component for both popup and panel.
   ...
   - :elements/temporary-banner: optional banner element to show below header (e.g., system feedback)"
...
[view-elements/app-header
 {:elements/wrapper-class "popup-header-wrapper"
  ...
  :elements/temporary-banner (when-let [fs-event (:ui/system-banner state)]
                               [system-banner fs-event])}]
```
...
```clojure
(defn panel-header [{:panel/keys [needs-refresh? system-banner-state]}]
  [view-elements/app-header
   {:elements/wrapper-class "panel-header-wrapper"
    ...
    :elements/temporary-banner (when system-banner-state [system-banner system-banner-state])}])
```

## 4. System Banner State Must Go Through Uniflow
...
(swap! !state assoc :ui/system-banner {:type event-type :message banner-msg})
(swap! !state update-in [:ui/system-bulk-names bulk-id] (fnil conj []) script-name)
```
...
### Update System Banner Listener in popup.cljs init!
...
(when show-banner?
  ...
  #_(dispatch! [[:uf/fx.defer-dispatch [[:popup/ax.clear-system-banner]] 3000]]))
